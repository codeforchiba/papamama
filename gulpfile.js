...
//    runSequence(['data-wards', 'data-middleSchool', 'data-elementary', 'data-school'], 'data-station', 'data-nursery', cb);
   runSequence(['data-middleSchool', 'data-elementary', 'data-school'], 'data-station', 'data-nursery', cb);
});
...
gulp.task("data-middleSchool", (cb) => {
  shapefile.read('data_org/A32-13_29.shp', {encoding: 'shift_jis'}, (err, json) => {
    if(err) {
      console.log(err);
    } else {
      json.features = json.features.filter((feature) => {
...
gulp.task("data-elementary", (cb) => {
  shapefile.read('data_org/A27-10_29-g_SchoolDistrict.shp', {encoding: 'shift_jis'}, (err, json) => {
    if(err) {
      console.log(err);
    } else {
      json.features = json.features.filter((feature) => {
...
gulp.task("data-school", (cb) => {
  shapefile.read('data_org/P29-13_29.shp', {encoding: 'shift_jis'}, (err, json) => {
    if(err) {
      console.log(err);
    } else {
...
//  奈良には区がないので無理やり
      var wardsJson = JSON.parse(fs.readFileSync('data/Elementary.geojson', 'utf8'));	
      json.features = json.features.filter((feature) => {
...
//  shapefile.read('data_org/P14-15_29.shp', {encoding: 'shift_jis'}, (err, json) => {
